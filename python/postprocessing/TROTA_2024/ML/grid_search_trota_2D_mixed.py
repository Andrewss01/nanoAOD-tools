...
usage = 'python3 grid_search_trota_2D_mixed -s TT,ZJ -i /eos/user/a/apuglia/thesis/training_dataset/trainingSet_PF_SV_10k.pkl -o True -g ./model_26_04_2025/ -m ./model_26_04_2025/model_26_04_2025.h5 -j ./model_26_04_2025/scores_26_04_2025.json -v True'
...
class trainer:
    ...
    def model_builder_tuning(self, hp):
        ...
        self.model = tf.keras.Model(inputs = [fj_inputs, jet_inputs, top_inputs, pfc_inputs], outputs = outputs)
    ...
    def model_builder(self, InputShape_FatJet, InputShape_Jet, InputShape_Top, InputShape_Pfc):
    ...
    def tune_hps(self, project_name, max_epochs, batch_size, factor =3):
        ...
        tuner.search({'fatjet': self.X_fatjet_train, 'jet': self.X_jet_train, 'top': self.X_top_train, 'pfc': self.X_pfc_train}, 
    ...
    def training(self, validation_split, epochs, batch_size, save_model = True, path_to_model = outModel):
        self.callbacks()
        self.model_builder(self.X_fatjet_train.shape[1], self.X_jet_train.shape[2], self.X_top_train.shape[1], self.X_pfc_train.shape[2])
        ...
        self.history = self.model.fit({'fatjet': self.X_fatjet_train, 'jet': self.X_jet_train, 'top': self.X_top_train, 'pfc': self.X_pfc_train}, 
    ...
    def predict(self, X_jet_train = None, X_fatjet_train = None, X_top_train = None, X_pfc_train = None, X_jet_test = None, X_fatjet_test = None, X_top_test = None, X_pfc_test = None):
        if (X_jet_train is None) and (X_fatjet_train is None) and (X_top_train is None) and (X_pfc_train is None):
            self.y_pred_train = self.model.predict({'fatjet': self.X_fatjet_train, 'jet': self.X_jet_train, 'top': self.X_top_train, 'pfc': self.X_pfc_train})
            self.y_pred_test  = self.model.predict({'fatjet': self.X_fatjet_test,  'jet': self.X_jet_test , 'top': self.X_top_test, 'pfc': self.X_pfc_test})
        else:
            y_pred_train = self.model.predict({'fatjet': X_fatjet_train, 'jet': X_jet_train, 'top': X_top_train, 'pfc': X_pfc_train})
            y_pred_test  = self.model.predict({'fatjet': X_fatjet_test , 'jet': X_jet_test , 'top': X_top_test, 'pfc': X_pfc_test })
    ...
    def evaluate(self, X_jet_test = None, X_fatjet_test = None, X_top_test = None, X_pfc_test = None, y_test = None):
        ...
        if (X_jet_test is None) and (X_fatjet_test is None) and (X_top_test is None) and (y_test is None) and (X_pfc_test is None) :
            self.eval_result = self.model.evaluate({"fatjet": self.X_fatjet_test, "jet": self.X_jet_test, "top": self.X_top_test, "pfc": self.X_pfc_test}, self.y_test)
            return self.eval_result
        else:
            eval_result      = self.model.evaluate({"fatjet": X_fatjet_test, "jet": X_jet_test, "top": X_top_test, "pfc": X_pfc_test}, y_test)
            return eval_result
...
trainer1.tune_hps(project_name = 'grid_search_trota_2D_mixed_'+label, max_epochs= 100, batch_size = 250)
trainer1.training(validation_split = 0.4, epochs = 200, batch_size = 250)
