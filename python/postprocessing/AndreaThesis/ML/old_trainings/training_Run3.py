...
sig = list(components)[0]
if no_tt_bkg == True:
    for cat in categories:
        idx_todrop = [i for i,x in enumerate(dataset[sig][cat][3] == 0) if x == True]
        dataset[sig][cat][0] = np.delete(dataset[sig][cat][0], idx_todrop, axis=0)    
        dataset[sig][cat][1] = np.delete(dataset[sig][cat][1], idx_todrop, axis=0)
        dataset[sig][cat][2] = np.delete(dataset[sig][cat][2], idx_todrop, axis=0)
        dataset[sig][cat][3] = np.delete(dataset[sig][cat][3], idx_todrop, axis=0)
...
best_hps_file        = "/eos/user/l/lfavilla/xAndreaCP/best_hps.json"
...
best_hps_path = path_to_outJson.replace('scores', 'best_hps')
with open(best_hps_path, 'w') as jsFile:
    json.dump(best_hyperparams)
