...
class trainer: 
    ...
    def model_builder(self):
        ...
        x = BatchNormalization()(fj_inputs)
        x = Dense(units = self.best_hps['fj_units'], activation = self.best_hps['fj_activation'], kernel_initializer = self.best_hps['fj_kernel_initializer'])(x)
        ...
        trainer = tf.keras.optimizers.Nadam(learning_rate = self.best_hps['learning_rate'])
        loss = tf.keras.losses.SparseCategoricalCrossentropy()
        self.model.compile(optimizer = trainer, loss = loss, metrics = ['accuracy'])
...
best_hps = trainer1.best_hps
trainer1.training(validation_split= 0.3, epochs= 1000, batch_size= 250)

print(f"BEST HPS FOUND:\n{best_hps}")
with open(f"{path_graphics}/best_hps_28_04_2025.json", "w") as jsFile:
