...
for c in components:
    for cat in categories:
        idx_truetop  = [i for i, x in enumerate(dataset[c][cat][5]==1) if x==True]
        idx_falsetop = [i for i, x in enumerate(dataset[c][cat][5]==0) if x==True]
...
X_top                     = np.concatenate([dataset[c][cat][4] for c in samples for cat in categories]) # here we use only the samples selected by the user
y                         = np.concatenate([dataset[c][cat][5] for c in samples for cat in categories]) # here we use only the samples selected by the user
