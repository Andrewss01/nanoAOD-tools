...
class trainer: 
    ...
    def split(self, test_size):
        self.X_jet_train, self.X_jet_test, self.X_fatjet_train, self.X_fatjet_test, self.X_top_train, self.X_top_test, self.X_pfc_train, self.X_pfc_test, self.X_sv_train, self.X_sv_test, self.y_train, self.y_test = train_test_split(self.X_jet, self.X_fatjet, self.X_top, self.X_pfc, self.X_sv, self.y,
    ...
    def model_builder_tuning(self, hp):
        ...
        InputShape_Sv = self.X_sv_train.shape[2]
        ...
        w = keras.layers.Dense(units = pfc_units, activation = pfc_activation, 
        ...
        k = keras.layers.Dense(units = sv_units, activation = sv_activation, 
                              kernel_initializer = sv_kernel_initializer)(k)
        ...
        dense_units = hp.Int('dense_units', min_value =5,max_value = 30, step = 5)
        ...
        self.model = tf.keras.Model( inputs = [jet_inputs, top_inputs, pfc_inputs, sv_inputs], outputs = outputs)
    ...
    def model_builder(self,  InputShape_Jet, InputShape_Top, InputShape_Pfc, InputShape_Sv):
        ...
        w = keras.layers.Dense(units = self.best_hps['pfc_units'], activation = self.best_hps['pfc_activation'], 
                                kernel_initializer = self.best_hps['pfc_kernel_initializer'])(w)
        w = GlobalAveragePooling1D()(w)
                            #   kernel_initializer = pfc_kernel_initializer)(w)
        
        k = BatchNormalization()(sv_inputs)
        k = keras.layers.Dense(units = self.best_hps['sv_units'], activation = self.best_hps['sv_activation'], 
                              kernel_initializer = self.best_hps['sv_kernel_initializer'])(k)
        k = GlobalAveragePooling1D()(k)
        print('x is: ', x)
        print('w is: ', w)
        print('y is: ', k)
        ...
        self.best_hps['pfc_units'], self.best_hps['pfc_activation'], self.best_hps['pfc_kernel_initializer'], self.best_hps['sv_units'], 
        self.best_hps['sv_activation'], self.best_hps['sv_kernel_initializer'])
        self.model = tf.keras.Model(inputs = [jet_inputs, top_inputs, pfc_inputs, sv_inputs], outputs = outputs)
    ...
    def tune_hps(self, project_name, max_epochs, batch_size, factor = 3):
        ...
        tuner.search({'jet': self.X_jet_train, 'top': self.X_top_train, 'pfc': self.X_pfc_train, 'sv': self.X_sv_train}, 
    ...
    def training(self, validation_split, epochs, batch_size, save_model = True, path_to_model = outModel ):     
        self.callbacks()
        self.model_builder(self.X_jet_train.shape[2], self.X_top_train.shape[1], self.X_pfc_train.shape[2], self.X_sv_train.shape[2])
        ...
        self.history = self.model.fit({ 'jet': self.X_jet_train, 'top': self.X_top_train, 'pfc': self.X_pfc_train, 'sv': self.X_sv_train}, 
    ...
    def predict(self, X_jet_train = None, X_fatjet_train = None, X_top_train = None, X_jet_test = None, X_fatjet_test = None, X_top_test = None, X_pfc_train = None, X_pfc_test = None, X_sv_train = None, X_sv_test = None):
        if (X_jet_train is None) and (X_fatjet_train is None) and (X_top_train is None) and (X_sv_train is None) and (X_pfc_train is None):
            self.y_pred_train = self.model.predict({ 'jet': self.X_jet_train, 'top': self.X_top_train, 'pfc': self.X_pfc_train, 'sv': self.X_sv_train})
            self.y_pred_test  = self.model.predict({ 'jet': self.X_jet_test , 'top': self.X_top_test, 'pfc': self.X_pfc_test, 'sv': self.X_sv_test})
        else:
            y_pred_train = self.model.predict({ 'jet': X_jet_train, 'top': X_top_train, 'pfc': X_pfc_train, 'sv': X_sv_train})
            y_pred_test  = self.model.predict({'jet': X_jet_test , 'top': X_top_test, 'pfc': X_pfc_test, 'sv':X_sv_test })
    ...
    def evaluate(self, X_jet_test = None, X_fatjet_test = None, X_top_test = None, X_pfc_test = None, X_sv_test = None, y_test = None):
        if (X_jet_test is None) and (X_fatjet_test is None) and (X_top_test is None) and (y_test is None) and (X_pfc_test is None) and (X_sv_test is None):
            self.eval_result = self.model.evaluate({ "jet": self.X_jet_test, "top": self.X_top_test, "pfc": self.X_pfc_test, "sv": self.X_sv_test}, self.y_test)
            return self.eval_result
        else:
            eval_result      = self.model.evaluate({"jet": X_jet_test, "top": X_top_test, "pfc": X_pfc_test, "sv": X_sv_test}, y_test)
            return eval_result
...
trainer1.tune_hps(project_name= 'tuning_resolved_'+label, max_epochs= 1000, batch_size= 250)
