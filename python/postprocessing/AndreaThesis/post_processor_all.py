...
p=PostProcessor(path,fnames,branchsel=None,modules=[MCweight_writer(), preselection(), GenPart_MomFirstCp(flavour = '-5,-4,-3,-2,-1,1,2,3,4,5,6,-6,24,-24'),Idx_PFC_SV(), deltaR_PF_SV(),
                                                    ...
                                                    postfix = '_Skim', outputbranchsel="%s/src/PhysicsTools/NanoAODTools/scripts/keep_and_drop_large.txt" % os.environ["CMSSW_BASE"], histFileName=path+"histOut_"+label+".root", histDirName="plots", haddFileName = path +label+'_cnn_model_sel.root')
p.run()
