...
example     = 'python3 training_multiclass.py -s TT,ZJ1,ZJ2 -i /eos/user/a/apuglia/SWAN_projects/thesis/CMSSW_14_1_7/src/training_dataset/trainingSet_10k.pkl' \
' -m /eos/user/a/apuglia/SWAN_projects/thesis/CMSSW_14_1_7/src/models/model_multiclass.h5 -j /eos/user/a/apuglia/SWAN_projects/thesis/CMSSW_14_1_7/src/trainings/score_tresholds_multiclass.json -g /eos/user/a/apuglia/SWAN_projects/thesis/CMSSW_14_1_7/src/graphics/Multiclass'
...
verbose                 = True
path_to_best_hps        = "/eos/user/l/lfavilla/xAndreaCP/best_hps.json"
...
#print('TO DROP', components_todrop)
...
class trainer:
    ...
    def model_builder(self, hp):
        ...
        fj_units = hp.Int('fj_units', min_value = 1, max_value = 5, step = 1)
        ...
        y = keras.layers.LSTM(units=self.BestHyperParameters["j_units"],
                              ...
                              dropout=self.BestHyperParameters["j_dropout"])(y)
    ...
    def plot_roc(self, name, labels, predictions, color="steelblue", linestyle="--", roc_model = 'OvR'):
        ...
        if roc_model == 'OvR':
            plt.savefig(f"{path_to_graphics_folder}/roc_curve_OvR.png")
            plt.savefig(f"{path_to_graphics_folder}/roc_curve_OvR.pdf")
        elif roc_model == 'OvO':
            plt.savefig(f"{path_to_graphics_folder}/roc_curve_OvO.png")
            plt.savefig(f"{path_to_graphics_folder}/roc_curve_OvO.pdf")
    ...
    #def train_test_roc_OvO(self):
        ...
        return results_ovr, results_ovo
    
    def tune_hyperparameters(self, epochs, batch_size):
        #epochs = 1000
        #batch_size = 250

        objective = kt.Objective('val_accuracy', direction = 'max')
        tuner = kt.Hyperband(self.model_builder, objective = objective, factor = 3, project_name = 'project_name')
        tuner.search_space_summary()
        self.callbacks()
        tuner.search({"fatjet": self.X_fatjet_train, "jet": self.X_jet_train, "top": self.X_top_train}, self.y_train,
                     validation_split=0.3, shuffle=True, callbacks=self.callback_list,
                     epochs=epochs, batch_size=batch_size, verbose=1)

        best_hps_gs = tuner.get_best_hyperparameters(num_trials= 1) 

        print(f'best hps found: \n {best_hps_gs[0].values}' )
# training parameters 
...
trainer1.tune_hyperparameters(epochs = 1000, batch_size= 250)
trainer1.training(validation_split=0.3, epochs=epochs, batch_size=batch_size, 
                     save_model=False, path_to_model=outModel, verbose=True)
...
ovr_res, ovo_res   = trainer1.test_roc()
