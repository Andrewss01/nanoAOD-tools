...
import sys
sys.path.append('/afs/user/a/apuglia/CMSSW_14_1_7/src/PhysicsTools')
...
from PhysicsTools.NanoAODTools.ML.NanoTopCandidate import *
...
if create_file_select:
    p=PostProcessor(".",files,branchsel=None,modules=[GenPart_MomFirstCp(flavour = '-5,-4,-3,-2,-1,1,2,3,4,5,6,-6,24,-24'), nanoprepro(), nanoTopcand(isMC = True)]
                    ,histFileName="histOut.root",histDirName="plots", postfix = '_Skim', outputbranchsel=os.path.abspath('PhysicsTools/NanoAODTools/scripts/keep_and_drop.txt'), maxEntries = 1000)
    p.run()
