...
from PhysicsTools.NanoAODTools.postprocessing.ML.idx_PFC import *
import os
...
if create_file_select:
    p=PostProcessor("/eos/user/a/apuglia/SWAN_projects/thesis/CMSSW_14_1_7/src/Datasets/",files,branchsel=None,modules=[GenPart_MomFirstCp(flavour = '-5,-4,-3,-2,-1,1,2,3,4,5,6,-6,24,-24'), Idx_PF(), nanoprepro(), nanoTopcand(isMC = True)]
                    ,histFileName="histOut.root",histDirName="plots", postfix = '_Skim', outputbranchsel="%s/src/PhysicsTools/NanoAODTools/scripts/keep_and_drop.txt" % os.environ["CMSSW_BASE"], maxEntries = 1000)
    p.run()
    
if select:
    p=PostProcessor("/eos/user/a/apuglia/SWAN_projects/thesis/CMSSW_14_1_7/src/Datasets/",files_toselect,branchsel=None,histFileName="histOut.root",histDirName="plots",  postfix = '_CutPt', outputbranchsel=os.path.abspath('PhysicsTools/NanoAODTools/scripts/keep_and_drop.txt'), maxEntries = 10000)
    p.run()
